-- scenes/filters.lua
-- Filter screen: select systems, genres, status, plus toggles.
...
local function rebuildColumns()
    ...
    -- Column 2: Genres
    local genre_items = {}
    table.insert(genre_items, {
        label = "Match: " .. (picker.genreMode() == "all" and "ALL GENRES" or "ANY GENRE"),
        kind  = "genre_mode",
    })
    for _, gn in ipairs(library.genres()) do
        table.insert(genre_items, {
            label = gn,
            kind  = "genre",
            value = gn,
    ...
    if #genre_items == 1 then
        table.insert(genre_items, {
            label = "(none yet - add on a pick with L1)",
            kind  = "info",
        })
    end
    table.insert(columns, { title = "GENRE", items = genre_items })
...
local function isChecked(item)
    ...
    elseif item.kind == "genre" then
        return picker.isGenreRequired(item.value)
...
local function toggleItem(item)
    ...
    elseif item.kind == "genre" then
        picker.setGenreRequired(item.value, not picker.isGenreRequired(item.value))
    elseif item.kind == "status" then
        picker.setStatusExcluded(item.value, not picker.isStatusExcluded(item.value))
    elseif item.kind == "genre_mode" then
        picker.toggleGenreMode()
        rebuildColumns()
    elseif item.kind == "surprise" then
...
function filters:update(dt)
    ...
    elseif input.pressed("y") then
        ...
        for _, item in ipairs(columns[col_idx].items) do
            ...
            elseif item.kind == "genre" then
                picker.setGenreRequired(item.value, false)
            elseif item.kind == "status" then
