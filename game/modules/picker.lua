...
local state = {
    ...
    genres_required    = {},   -- set: genre -> true (AND/OR via genre_mode)
    genre_mode        = "any",-- "any" or "all"
...
function picker.setGenreRequired(genre, required)
    state.genres_required[genre] = required and true or nil
end

function picker.isGenreRequired(genre)
    return state.genres_required[genre] == true
end

function picker.toggleGenreMode()
    state.genre_mode = (state.genre_mode == "any") and "all" or "any"
end

function picker.genreMode()
    return state.genre_mode
end
...
function picker.eligible()
    ...
    for _, g in ipairs(games) do
        ...
        if include and not state.surprise_me then
            ...
            -- Tag filter
            if include and next(state.genres_required) then
                local game_genres = {}
                for _, t in ipairs(g.genres or {}) do game_genres[t] = true end
                if state.genre_mode == "all" then
                    for gtag in pairs(state.genres_required) do
                        if not game_genres[gtag] then include = false; break end
                    end
                else
                    local match = false
                    for gtag in pairs(state.genres_required) do
                        if game_genres[gtag] then match = true; break end
                    end
